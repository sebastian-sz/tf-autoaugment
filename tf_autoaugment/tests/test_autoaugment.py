...
class TestAutoAugment(parameterized.TestCase):
    ...
    def test_autoaugment_working_in_eager_mode(self, tf_function: bool):
        autoaugment = AutoAugment(policy="_debug", tf_function=tf_function)

        transformed_images = autoaugment(self.mock_inputs)

        with self.assertRaises(AssertionError):
            np.testing.assert_array_equal(self.mock_inputs, transformed_images)

    @parameterized.named_parameters(NAMED_TEST_PARAMS)
    def test_autoaugment_working_in_graph_mode(self, tf_function: bool):
        autoaugment = AutoAugment(policy="_debug", tf_function=tf_function)

        dataset = tf.data.Dataset.from_tensor_slices([self.mock_inputs])
        dataset = dataset.map(lambda x: autoaugment(x))
