"""Code for Translate Y Transform."""
...
class TranslateY(BaseTransform):
    """Implements Translate Y Transform."""
    ...
    def __call__(self, images: tf.Tensor, level: tf.Tensor) -> tf.Tensor:
        """Run translate_y function on image batch."""
        level = self._parse_level(level)
        return self.translate_y(images=images, replace=self.fill_value, pixels=level)
